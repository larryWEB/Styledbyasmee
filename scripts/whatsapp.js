...
document.addEventListener("DOMContentLoaded", () => {
  // Initialize WhatsApp chat feature
  initWhatsAppChat()
})
...
function initWhatsAppChat() {
  ...
  const agents = [
    ...
    },
    {
      ...
      phone: "2345678901", // Replace with actual phone number
      avatar: "S",
      status: "online",
    },
    {
      ...
      phone: "3456789012", // Replace with actual phone number
      avatar: "M",
      status: "offline",
    },
    {
      ...
      phone: "4567890123", // Replace with actual phone number
      avatar: "J",
      status: "online",
    },
  ]

  // Create WhatsApp button and popup
  createWhatsAppElements(agents)

  // Add event listeners
  setupWhatsAppEvents()
}

function createWhatsAppElements(agents) {
  // Create container
  ...
  whatsappPopup.innerHTML = `
    ...
    <div class="whatsapp-agents">
      ${agents
        .map(
          (agent) => `
        <a href="https://wa.me/${agent.phone}" target="_blank" class="whatsapp-agent">
          <div class="agent-avatar">${agent.avatar}</div>
          <div class="agent-info">
            <div class="agent-name">${agent.name}</div>
            <div class="agent-role">${agent.role}</div>
          </div>
          <div class="agent-status ${agent.status === "offline" ? "offline" : ""}"></div>
        </a>
      `,
        )
        .join("")}
  ...
  whatsappContainer.appendChild(whatsappPopup)
